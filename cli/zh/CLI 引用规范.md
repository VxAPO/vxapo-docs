...
> **定位**：CLI 是**开发者工具**（overview/项目概览.md「三层分离」五节），依赖 vxapo-driver（as library）经
> **安装层 API** 操作设备，不触碰 pipeline/RT；面向调试自动化与 P0 端到端验证，不面向终端用户。
> **依据**：源码实读 `D:\APO_Project\VxAPO\vxapo-cli`（v0.3.2）+ `D:\APO_Project\VxAPO\vxapo-driver`（2026-08-13）。

---

## 一、现有 CLI 现状（源码实读）

### 1.1 文件结构（vxapo-cli/src，9 文件）

```
vxapo-cli/
├── Cargo.toml          # 依赖 winreg = "0.52" + vxapo-driver（path = "../vxapo-driver"）+ windows
├── src/
│   ├── main.rs         # 入口：无参数 → 交互菜单；带子命令 → 命令行自动化
│   ├── commands.rs     # 命令层：list/status、install、uninstall、config、snapshot、help
│   ├── app.rs          # 查看模式 App 状态（endpoints 列表 + refresh）
│   ├── display.rs      # 端点列表/详情打印
│   ├── endpoint.rs     # 端点查询（独立 winreg 枚举，查看模式保留）
...
3. **槽位失守检测接入（v8.5 产品意图）**：CLI 启动 / 切换设备时，检测 `DeviceInfo.install_mode`
   的 premix/postmix 槽位值 ≠ VxAPO CLSID → 提示「需重新安装」→ 确认 → `install_endpoint`
   （内部先 `child_apo_key_exists` 判定全量/非全量 → 失守重装覆盖备份 childapo 最新前任）

### Phase B：核心命令（P0-7 端到端）

#### 4.4.1 命令参数定义（补充 2026-08-04）
...
### Phase C：快照 = 变更对比 + 基线保持（补充 2026-08-04）+ 注册表转储保留

**快照的真实功能 = 列出更改项目（diff）**，非纯备份。

> **范围限定（v8.8 范围修正）**：快照**只捕获注册表改动**——config 文件的比对**不属于 CLI 快照**，
...
## 五、CLI 行为流（补充 2026-08-04——安装/验证/卸载，具体到函数、层次分明）
...
main
 └─ cli::uninstall(args)
     ...
     └─ 7. 可选 config 清理：删除 C:\ProgramData\VxAPO\{guid}\config.toml（确认，v8.9）
```

### 5.4 命令状态流（补充 2026-08-04）

> **目标**：明确每个命令执行**需要判断什么**、命令执行把**输入窗口变成哪几种状态**、
> 每种状态**可执行的命令**、以及**状态切换逻辑**——保证每步骤**绝对严格 + 可读反馈 + 错误检验**（要求）。
...
- overview/项目概览.md 五节（三层分离）/ 七节（槽位失守检测 v8.5）/ 十一（config.toml 模型）——CLI 行为上位约束
