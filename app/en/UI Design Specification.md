...
- Custom pointer-level drag engine (`useDragSort`): 500 ms enter debounce, 400/320 ms layout animations, 80 ms settle buffer, 48 px outside distance, 430 ms fly landing. Slots shift with the page scroll during a drag (the hit test re-runs from the last pointer position), the landing point is re-measured after the synchronous reorder, and the flight animates `x`/`y` (composited, snapPx-aligned) with a static size. The flight's basis is the landing point itself, with path offsets measured from it and a final frame of 0, so a cached transform rewrite cannot shift the landing; shortly after the motion ends the flight drops `transform`/`will-change` and returns to normal painting (a layer that stays composited renders text at a different raster/antialiasing quality). The release path applies its layout silently and defers `finalizeDrop` out of the `pointerup` handler so the input pipeline is not blocked.
