...
- Custom pointer-level drag engine (`useDragSort`): 500 ms enter debounce, 400/320 ms layout animations, 80 ms settle buffer, 48 px outside distance, 430 ms fly landing. Slots shift with the page scroll during a drag (the hit test re-runs from the last pointer position), the landing point is re-measured after the synchronous reorder, and the flight animates `x`/`y` (composited, snapPx-aligned) with a static size.
