...
- **落地交接**：位置动画跑完（`FLY_MOVE_MS = 216`）后 `FLY_HANDOVER_MS`（+40ms）把已到达的落点写回
  `left`/`top`、撤掉 `transform` 与 `will-change`，让副本从合成层回到常规绘制——合成层的文字抗锯齿与
  栅格分辨率与常规层不同，整段停在合成层上、飞行停下后就看得出"分辨率下降"。
- **松手不放重活**：拖动期间 `applyLayout` 靠 500ms 防抖把整页重渲染摊开，松手没有防抖。因此松手时
  那一记布局应用走 `silent`（不驱动徽标、不触发重渲染），`finalizeDrop` 也推进定时器执行，不在
