...
| 设备页切换 | 淡出 / 淡入各 `280ms`（`DEVICE_FADE_MS`）：**数据源滞后一拍**——`useDeviceSwapFade` 先让旧数据淡出，再换 `selectedGuid` 并淡入，两段严格串行；元素不按设备重挂载（`AnimatePresence mode="wait"` 只保证"元素"先退后进，内容由 store 驱动会在旧页淡出期间就换成新设备数据） |
