...
- **飞行副本跟随内容滚动**：副本挂在 `fixed` 层（在滚动容器之外），落点是创建那一刻的视口坐标；
  用户一滚，整条弧线就会被内容甩掉。飞行期间监听滚动，按活动视图舞台的位移**同向**平移副本基准
  （命令式写 `left`/`top`，不重渲染），弧线跟着内容走，终点始终压在目标槽位上。
  方向符号与拖拽中的槽位平移（`shiftSlots`）一致：落点是视口坐标，内容位移 `dy` 后同一位置的新视口
  坐标就是「落点 + dy」；取相反号会让副本朝滚动反方向跑，与目标槽位差出两倍滚动量。
- **跟手与滚动合帧**：`pointermove` / `scroll` 只记录最新状态，定位与命中在 `requestAnimationFrame`
  ...
  飞行起点必须是指针最后停下的位置。
- **飞行补偿不能合帧**（与上一条的区别）：滚动位移在事件所在那一帧就参与绘制，补偿若推到下一帧
  rAF，滚动的那一帧副本仍被内容带偏（看起来「滚一下动画抖一下」）。所以飞行期间的滚动补偿在事件里
  **同步**写；拖拽中的槽位平移可以合帧——它只影响「哪张卡让位」的判定，晚一帧肉眼看不出来。
