...
- **补间只在像素空间进行，不换算量程、也不夹边界**：曲线与坐标轴是两件独立的事——纵轴量程
  （`yTop`/`yBottom`，按峰值自适应、每 2dB 跳一档）该瞬时变就瞬时变（网格由 React 直接重画），
  曲线只管从「旧形状所在的屏幕位置」平滑过渡到「新形状所在的屏幕位置」。起点与终点本就都落在绘图区内，
  插值自然也在区内。**曾经**改成「先按量程换算 dB、再钳到绘图区」：量程收窄时旧形状换算后必然越界，
  于是有一段被压在顶边成直线、再变回曲线，观感极差（已回退）。
