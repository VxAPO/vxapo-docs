...
lookahead 峰值限幅；Wide v9.10 线性相位 FIR 分频加宽；Aural v9.9 tanh
软饱和 + 电平跟随）。
...
- `Wide: Intensity 0.354331`
  - Intensity [0, 1]（默认 0.354331；0 时严格直通）；200 Hz 1024 点线性相位
    FIR 分频：低频支路（<200 Hz）完全不处理，高频支路 M/S 幂指数加宽 +
    tanh 软限幅；只处理前两个选中通道，单声道直通；延迟 511 采样（上报）。
...
- `process` 运行在 RT 线程：零分配、无锁、无 I/O；`latency()` 返回 0。
  （Wide v9.10 例外：按实际 FIR 延迟上报 511 采样。）
