...
/// | 2 | 端点实例 ID 以 `BTHENUM` / `BTHLE` 开头（蓝牙音频设备） | SfxMfx（Win11 蓝牙组合，EFX 无效） |
/// | 3 | 否则（现代驱动默认） | SfxEfx |
///
/// # 参数
///
...
/// - `has_bluetooth`：端点实例 ID 以 `BTHENUM` / `BTHLE` 开头。由 `info.rs` 从
///   `Properties` 子键的 `PKEY_DEVICE_INSTANCE_ID` 读出后经 `is_bluetooth_instance_id` 判定。
