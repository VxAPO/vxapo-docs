...
> 在 10ms 过渡窗口下，"阻塞到过渡完成的解析延迟"完全可忽略（调用方决策，R2 理由）。
```
...
fn Initialize(&self, cb_data_size: u32, pby_data: *mut u8) -> HRESULT {
    ...
    //    e. config.toml 不存在 → 写入默认 passthrough（空文件或仅注释行）
...
> - 监控对象 = **config_path 父目录**（`C:\ProgramData\VxAPO\{GUID}`，v8.9），**非 config.toml 文件本身**——
...
- **config 默认写入**：config.toml 不存在时写入默认 passthrough（空文件或仅注释行，
  DLL 不创建预设——预设由 App/CLI 管理，dll 只兜底 passthrough）
- **无设备 GUID 兜底**：`PKEY_AudioEndpoint_GUID` 提取失败 / 值为空（无端点属性）时，
  回退单实例共用路径 `C:\ProgramData\VxAPO\_default\config.toml`（日志告警，不阻断初始化）
- **目录创建失败兜底**：`create_dir_all` 失败 → 回退**固定单实例路径**
  `C:\ProgramData\VxAPO\config.toml`（`DEFAULT_CONFIG_PATH` 常量，非 `_default` 子目录）
...
fn hot_reload(&self) {
    ...
    //    正常 config.toml < 1KB；超限视为异常文件，保留当前链 + 日志告警。
...
- **槽位失守检测（v8.1 定稿 + v8.5 精确定性，调用方决策——应用层，非 watcher）**：CLI/GUI 在 **启动 / 切换设备**
...
> **v7.5 变更（确定正式 GUID）**：CLSID 由占位值替换为正式生成值——
